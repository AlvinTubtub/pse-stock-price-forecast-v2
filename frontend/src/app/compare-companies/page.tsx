...
export default async function CompareCompaniesPage() {
  ...
  if (siteConfig.features.compareCompanies === false) {
    return (
      <div className="bg-dark-card border border-dark-border rounded-2xl p-12 text-center space-y-4 max-w-lg mx-auto">
        <p className="text-4xl">⚖️</p>
        <h1 className="text-xl font-bold text-white">Compare Stocks Feature Temporarily Disabled</h1>
        <p className="text-xs text-slate-400 leading-relaxed">
          This analytical comparison module has been temporarily disabled by administrators for scheduled maintenance.
        </p>
        <a
          href="/companies"
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-2 pb-4 border-b border-dark-border/60">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-400 mt-1">
            Select 2 to 4 tracked Philippine Stock Exchange securities to evaluate side-by-side forecasts, expected movements, and validation metrics.
          </p>
        </div>
      ...
      <CompareCompaniesClient companies={companies} metrics={metrics} />
    </div>
  );
}
